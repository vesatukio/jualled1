...
(function(){
  ...
  async function rest(path,options={}){const r=await fetch(SUPABASE_URL+path,{...options,headers:{...headers,...(options.headers||{})},cache:"no-store"});let body=null;try{body=await r.json()}catch(_){}if(!r.ok){const message=body?.message||body?.hint||body?.details||body?.error||("HTTP "+r.status);throw new Error(message)}return body}
  ...
  async function submitOrder(e){
    ...
    try{
      ...
      const createdRows=await rest("/rest/v1/pesanan",{method:"POST",body:JSON.stringify(payload)});
      const created=Array.isArray(createdRows)?createdRows[0]:createdRows;
      if(!created?.id)throw new Error("Server tidak mengembalikan ID pesanan.");
      const details=cart.map(i=>{const pid=Number(i.id);return{pesanan_id:created.id,produk_id:Number.isFinite(pid)&&pid>0?pid:null,nama_produk:i.nama||"Produk",qty:Number(i.qty)||1,harga_saat_beli:Number(getPrice(i))||0}});
      try{await rest("/rest/v1/detail_pesanan",{method:"POST",body:JSON.stringify(details)})}catch(detailError){
        console.error("Detail order error",detailError);
        alert("Pesanan utama sudah dibuat tetapi detail produk gagal disimpan. Nomor pesanan: "+orderNo+". Hubungi admin sebelum membuat pesanan ulang.");
        throw detailError;
      }
    ...
    }catch(err){console.error("Supabase order error",err);if(!String(err.message||"").includes("Nomor pesanan"))alert("Pesanan gagal disimpan: "+(err.message||err));}
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    if(b){b.textContent="Checkout";b.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();openCheckout()},{capture:true})}
