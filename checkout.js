...
(function(){
  ...
  async function rpc(name,body){const r=await fetch(SUPABASE_URL+"/rest/v1/rpc/"+name,{method:"POST",headers,body:JSON.stringify(body),cache:"no-store"});let j=null;try{j=await r.json()}catch(_){}if(!r.ok)throw new Error(j?.message||j?.hint||j?.details||j?.error||("HTTP "+r.status));return j}
  ...
  async function submitOrder(e){
    ...
    try{
      ...
      localStorage.setItem("dutaled_last_order",JSON.stringify(order));let old=[];try{old=JSON.parse(localStorage.getItem("dutaled_orders")||"[]")||[]}catch(_){}localStorage.setItem("dutaled_orders",JSON.stringify([order,...old].slice(0,50)));
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    const b=document.getElementById("checkoutButton");if(b){b.textContent="Checkout";b.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();openCheckout()},{capture:true})}
    ...
    document.querySelectorAll("[data-checkout-close]").forEach(x=>x.addEventListener("click",closeCheckout));document.getElementById("checkoutForm")?.addEventListener("submit",submitOrder)
  })
})();