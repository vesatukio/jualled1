...
(() => {
  ...
  async function trackVisitor(){
    try{
      ...
      const device=/Mobi|Android|iPhone|iPad/i.test(navigator.userAgent)?"mobile":/Tablet/i.test(navigator.userAgent)?"tablet":"desktop";
      await originalFetch(SUPABASE_URL+"/rest/v1/visitor_logs",{method:"POST",headers:{apikey:SUPABASE_KEY,Authorization:"Bearer "+SUPABASE_KEY,"Content-Type":"application/json","Prefer":"return=minimal"},body:JSON.stringify({visitor_id:visitorId,session_id:sessionId,path:location.pathname+location.search,title:document.title||null,referrer:document.referrer||null,device_type:device,user_agent:navigator.userAgent.slice(0,300)})});
    }catch(error){console.warn("Visitor analytics:",error?.message||error)}
  }
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>setTimeout(trackVisitor,300));else setTimeout(trackVisitor,300);
})();