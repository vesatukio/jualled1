...
(function(){
  ...
  async function submit(e){
    ...
    const f=e.currentTarget;if(!f.reportValidity())return;const c=cart();if(!c.length){alert('Keranjang kosong.');return;}
    const d=Object.fromEntries(new FormData(f).entries());const orderNo='DL'+new Date().toISOString().replace(/\D/g,'').slice(0,14)+Math.floor(Math.random()*90+10);const btn=f.querySelector('.checkout-submit');if(btn){btn.disabled=true;btn.textContent='Menyimpan pesanan...';}
    try{
      ...
      localStorage.setItem('dutaled_last_order',JSON.stringify(o));localStorage.setItem(ORDERS_KEY,JSON.stringify([o,...orders()].slice(0,50)));clearCart();close();document.dispatchEvent(new CustomEvent('dutaled:order-created',{detail:o}));
      if(typeof window.showOrders==='function')window.showOrders();else alert('Pesanan berhasil dibuat: '+o.orderNo);
