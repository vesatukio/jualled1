...
(function(){
  ...
  function open(){if(!cart().length){alert('Keranjang masih kosong.');return false;}closeCart();build();render();const m=document.getElementById('checkoutModal');m.classList.add('show');m.setAttribute('aria-hidden','false');document.body.classList.add('checkout-open');return true;}
  ...
  async function submit(e){
    ...
    const c=cart();if(!c.length){alert('Keranjang kosong.');return;}
    const d=Object.fromEntries(new FormData(f).entries());
    const orderNo='DL'+new Date().toISOString().replace(/\D/g,'').slice(0,14)+Math.floor(Math.random()*90+10);
    const btn=f.querySelector('.checkout-submit');if(btn){btn.disabled=true;btn.textContent='Menyimpan pesanan...';}
    try{
      ...
      const r=await fetch(SUPABASE_URL+'/rest/v1/rpc/create_public_order',{method:'POST',headers,body:JSON.stringify({p_order:payload,p_items:items}),cache:'no-store'});
      let j=null;try{j=await r.json();}catch(_){ }
      if(!r.ok)throw new Error(j?.message||j?.details||j?.hint||j?.error||('HTTP '+r.status));
      if(!j?.success||!j?.id)throw new Error('Database tidak mengembalikan ID pesanan.');
      ...
      localStorage.setItem('dutaled_last_order',JSON.stringify(o));
      localStorage.setItem(ORDERS_KEY,JSON.stringify([o,...orders()].slice(0,50)));
      clearCart();close();
      document.dispatchEvent(new CustomEvent('dutaled:order-created',{detail:o}));
      if(typeof window.showOrders==='function'){window.showOrders();}
      else{alert('Pesanan berhasil dibuat: '+o.orderNo);}
    }catch(err){console.error(err);alert('Pesanan gagal disimpan: '+(err.message||err));}
  ...
  function hook(){
    ...
    if(b&&!b.dataset.hotfix){b.dataset.hotfix='1';b.onclick=null;b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();open();},true);}
    if(typeof window.openCheckout!=='function')window.openCheckout=open;
  }
