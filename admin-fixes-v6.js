/* DUTA LED - Admin fixes v10 - syntax-safe */
(function(){
  'use strict';
  ...
  function cleanupTabs(){document.querySelectorAll('.tabs button,.tabs a').forEach(b=>{const t=String(b.textContent||'').trim().toLowerCase();if(t.includes('promo header')||t==='💬 saran & kritik'||t==='saran & kritik'||t==='saran dan kritik')b.remove()})}
  function mountOrderTabs(){const host=$('orderStatusTabs'),list=$('orderList');if(!host||!list)return;const cards=Array.from(list.querySelectorAll('.order')),active=window.__dutaFinalStatus||'ALL';host.innerHTML=STATUS.map(function(pair){const key=pair[0],label=pair[1],n=key==='ALL'?cards.length:cards.filter(function(c){return statusKey(c.querySelector('.status')?.textContent)===key}).length;return '<button type="button" class="order-status-tab '+(key===active?'active':'')+'" data-final-status="'+key+'">'+label+'<b>'+n+'</b></button>'}).join('');host.querySelectorAll('.order-status-tab').forEach(function(b){b.onclick=function(){window.__dutaFinalStatus=b.dataset.finalStatus;mountOrderTabs()}});cards.forEach(function(c){const k=statusKey(c.querySelector('.status')?.textContent);c.style.display=active==='ALL'||k===active?'':'none'})}
  ...
  function run(){cleanupTabs();styleStatus();mountOrderTabs()}
